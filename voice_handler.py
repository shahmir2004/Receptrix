...
Phase D: Accepts business_id to load per-tenant config from Supabase.
"""
import os
import json
import re
...
def get_ai_provider() -> AIProvider:
    ...
    if provider_name not in providers:
        logger.warning("Unknown provider '%s', defaulting to Groq", provider_name)
        provider_name = "groq"
...
class AIVoiceHandler:
    ...
    def __init__(self, business_id: Optional[str] = None):
        ...
        logger.info("AI Voice Handler initialized with provider: %s, business: %s", type(self.provider).__name__, business_id or 'default')
...
Return ONLY the JSON object, no other text."""
    ...
    def generate_response(
        ...
        caller = get_or_create_caller(caller_phone, business_id=self.business_id)
        if caller.name:
        ...
        if extracted.get("name"):
            ...
            update_caller_name(caller_phone, extracted["name"], business_id=self.business_id)
        ...
        try:
            ...
            logger.error("AI response error: %s", e, exc_info=True)
            return VoiceResponse(
    ...
    def _build_context_info(self, context: dict, extracted: dict) -> str:
        ...
        if context.get("requested_date"):
            try:
                ...
                logger.error("Error getting availability: %s", e)
    ...
    def _attempt_booking(self, context: dict) -> dict:
        ...
        try:
            ...
            if not check_time_slot_available(date, time, service.duration, business_id=self.business_id):
                # Find alternatives
                day_name = datetime.strptime(date, "%Y-%m-%d").strftime("%A").lower()
                working_hours = getattr(self.config.working_hours, day_name, "Closed")
                alternatives = get_available_slots(date, working_hours, service.duration, business_id=self.business_id)[:3]
            ...
            # Get caller
            caller = get_caller_by_phone(context["caller_phone"], business_id=self.business_id)
...
# ============ Handler Registry (per business_id) ============
# One handler per business to avoid cross-tenant config leaks.
_handlers: Dict[Optional[str], AIVoiceHandler] = {}


def get_voice_handler(business_id: Optional[str] = None) -> AIVoiceHandler:
    """Get or create the voice handler for a specific business."""
    global _handlers
    if business_id not in _handlers:
        _handlers[business_id] = AIVoiceHandler(business_id=business_id)
    return _handlers[business_id]
